    ...
    return Column(
      ...
      children: [
        ...
        for (var row = 0; row < rowCount; row++) ...[
          if (row > 0) const SizedBox(height: ParentPortalSpacing.xs),
          Row(
            children: [
              for (var col = 0; col < 7; col++) ...[
                if (col > 0) const SizedBox(width: ParentPortalSpacing.xs),
                Expanded(
                  child: _cell(
                    context,
                    cellIndex: row * 7 + col,
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(12),
          color: isSelected
              ? ParentPortalColors.accent(context)
              : isToday
                  ? ParentPortalColors.primarySoft(context)
                  : Colors.transparent,
          border: Border.all(
            color: isToday && !isSelected
                ? ParentPortalColors.accent(context)
                : ParentPortalColors.cardBorder(context),
          ),
        ),
        child: Column(
